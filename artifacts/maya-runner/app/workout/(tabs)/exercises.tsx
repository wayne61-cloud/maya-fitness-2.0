...
export default function ExercisesScreen() {
  ...
  const filtered = EXERCISES.filter((e) => {
    ...
    const matchSearch = !search || e.name.toLowerCase().includes(search.toLowerCase()) || e.muscleGroup.toLowerCase().includes(search.toLowerCase());
    ...
    >
      ...
      <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.groupScrollContent}>
        {MUSCLE_GROUPS.map((g) => (
          <TouchableOpacity
            ...
            activeOpacity={0.75}
          >
            <Text style={[styles.groupChipText, { color: selectedGroup === g ? "#fff" : colors.foreground }]}>{g}</Text>
          </TouchableOpacity>
        ))}
      </ScrollView>
      ...
      {filtered.map((ex) => (
        <TouchableOpacity
          ...
          activeOpacity={0.8}
        >
          <View style={[styles.exerciseIcon, { backgroundColor: WORKOUT_ORANGE + "18" }]}>
            <Ionicons name="barbell-outline" size={20} color={WORKOUT_ORANGE} />
          </View>
          <View style={{ flex: 1, gap: 3 }}>
            <View style={styles.exerciseRow}>
              <Text style={[styles.exerciseName, { color: colors.foreground }]}>{ex.name}</Text>
              <View style={[styles.levelBadge, { backgroundColor: LEVEL_COLORS[ex.level] + "20" }]}>
            ...
            <View style={styles.exerciseMeta}>
              ...
              {ex.secondaryMuscles.slice(0, 2).map((m) => (
                <Text key={m} style={[styles.secondaryMuscle, { color: colors.mutedForeground }]}>· {m}</Text>
              ))}
            </View>
            <View style={styles.exerciseDetails}>
              <View style={styles.detailItem}>
                <Ionicons name="barbell-outline" size={11} color={colors.mutedForeground} />
                <Text style={[styles.detailText, { color: colors.mutedForeground }]}>{ex.equipment}</Text>
              </View>
              <View style={styles.detailItem}>
                <Ionicons name="flame-outline" size={11} color={colors.mutedForeground} />
                <Text style={[styles.detailText, { color: colors.mutedForeground }]}>{ex.calPerMin} kcal/min</Text>
              </View>
            </View>
          </View>
...
const styles = StyleSheet.create({
  ...
  exerciseIcon: { width: 44, height: 44, borderRadius: 13, alignItems: "center", justifyContent: "center" },
  exerciseRow: { flexDirection: "row", alignItems: "center", gap: 8, justifyContent: "space-between" },
  ...
  exerciseMeta: { flexDirection: "row", alignItems: "center", gap: 4, flexWrap: "wrap" },
