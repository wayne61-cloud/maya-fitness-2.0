    ...
    >
      <View style={[styles.filterRow, { backgroundColor: colors.card, borderColor: colors.border }]}>
        {(["all", "week", "month"] as const).map((f) => (
          <TouchableOpacity
            key={f}
            style={[styles.filterBtn, filter === f && { backgroundColor: RUNNER_RED }]}
            onPress={() => setFilter(f)}
            activeOpacity={0.7}
          >
      ...
      <View style={styles.summaryRow}>
        <View style={[styles.summaryCard, { backgroundColor: colors.card, borderColor: colors.border }]}>
          <Text style={[styles.summaryVal, { color: RUNNER_RED }]}>
        ...
        <View style={[styles.summaryCard, { backgroundColor: colors.card, borderColor: colors.border }]}>
          <Text style={[styles.summaryVal, { color: colors.accent }]}>{formatTime(totalTime)}</Text>
          <Text style={[styles.summaryLabel, { color: colors.mutedForeground }]}>Durée</Text>
        </View>
        <View style={[styles.summaryCard, { backgroundColor: colors.card, borderColor: colors.border }]}>
          <Text style={[styles.summaryVal, { color: colors.success }]}>{filtered.length}</Text>
...
const styles = StyleSheet.create({
  ...
  filterRow: { flexDirection: "row", borderRadius: 12, padding: 4, borderWidth: 1, gap: 4 },
  ...
  empty: { borderRadius: 16, borderWidth: 1, borderStyle: "dashed", padding: 32, alignItems: "center", gap: 10, marginTop: 20 },
