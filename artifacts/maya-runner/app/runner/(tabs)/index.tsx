...
function StatBox({ value, unit, label, color }: { value: string; unit?: string; label: string; color: string }) {
...
export default function RunnerHome() {
  ...
  const weeklyGoalPct = Math.min(Math.round((weeklyDistance / Math.max(profile.weeklyGoal, 1)) * 100), 100);
    ...
    >
      ...
      <View style={[styles.progressCard, { backgroundColor: colors.card, borderColor: colors.border }]}>
        ...
        <View style={[styles.progressTrack, { backgroundColor: colors.secondary }]}>
          <View
            style={[styles.progressFill, { width: `${weeklyGoalPct}%`, backgroundColor: RUNNER_RED }]}
          />
        </View>
      ...
      <View style={styles.statsRow}>
        <StatBox value={weeklyDistance.toFixed(1)} unit="km" label="Cette semaine" color={RUNNER_RED} />
        <StatBox value={String(weeklyRuns.length)} label="Sorties" color={colors.accent} />
        <StatBox value={totalDistance.toFixed(1)} unit="km" label="Total" color={colors.foreground} />
      </View>
      ...
      {recentRuns.length > 0 ? (
        <>
          <View style={styles.sectionHeader}>
            <Text style={[styles.sectionTitle, { color: colors.foreground }]}>Dernières courses</Text>
        ...
        <View style={[styles.emptyState, { borderColor: colors.border }]}>
          ...
          <Text style={[styles.emptyTitle, { color: colors.foreground }]}>Pas encore de courses</Text>
          <Text style={[styles.emptyText, { color: colors.mutedForeground }]}>
            Lance ta première course pour commencer ton aventure de coureur !
          </Text>
        </View>
      )}
    </ScrollView>
...
const styles = StyleSheet.create({
  ...
  startBtn: {
    ...
    borderRadius: 30, alignSelf: "flex-start", marginTop: 6,
  },
