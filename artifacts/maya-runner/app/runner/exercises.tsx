...
export default function RunnerExercisesScreen() {
  const colors = useColors();
  const insets = useSafeAreaInsets();
  const params = useLocalSearchParams<{ category?: string }>();

  const [search, setSearch] = useState("");
  const [selectedCat, setSelectedCat] = useState<string>(params.category ?? "Tout");
      ...
      >
        ...
        >
          {categories.map((cat) => {
            ...
            const color = isAll ? RUNNER_RED : (CATEGORY_COLORS[cat] ?? RUNNER_RED);
            ...
            >
              ...
              <View style={{ flex: 1, gap: 4 }}>
                ...
                <View style={styles.metaRow}>
                  ...
                  <View
                    style={[styles.levelBadge, { backgroundColor: LEVEL_COLORS[ex.level] + "18" }]}
                  >
